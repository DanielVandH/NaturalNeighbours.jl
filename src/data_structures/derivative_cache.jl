...
function DerivativeCache(tri::Triangulation{P,Ts,I,E,Es,BN,BNM,B,BIR,BPL};
) where {P,Ts,I,E,Es,BN,BNM,B,BIR,BPL}
    iterated_neighbourhood = Set{I}()
    F = number_type(tri)
    linear_matrix = ElasticMatrix{F, Vector{F}}(undef, 3, 0)
    quadratic_matrix = ElasticMatrix{F, Vector{F}}(undef, 10, 0)
    rhs_vector = zeros(F, 0)
    linear_sol = zeros(F, 3)
    quadratic_sol = zeros(F, 10)
    sizehint_m = 2^4
    sizehint!(iterated_neighbourhood, sizehint_m)
    sizehint!(linear_matrix, (3, sizehint_m))
    sizehint!(quadratic_matrix, (10, sizehint_m))
