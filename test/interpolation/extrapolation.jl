...
@testset "Basic extrapolation" begin
    ...
    @test isnan(itp(getx(p), gety(p); method=:triangle, project=false))
    @test isnan(itp(getx(p), gety(p); method=:sibson, project=false))
    @test isnan(itp(getx(p), gety(p); method=:laplace, project=false))
    @test isnan(itp(getx(p), gety(p); method=Sibson(1), project=false))
    @test isnan(itp(1.8, 0.7; method=:triangle, project=false))
    @test isnan(itp(1.8, 0.7; method=:sibson, project=false))
    @test isnan(itp(1.8, 0.7; method=:laplace, project=false))
    @test isnan(itp(1.8, 0.7; method=Sibson(1), project=false))

    ∂ = differentiate(itp, 1)
    @test all(isnan, ∂(getx(p), gety(p); project=false))
    ∂ = differentiate(itp, 2)
    @test all(isnan, ∂(getx(p), gety(p); project=false)[1])
    @test all(isnan, ∂(getx(p), gety(p); project=false)[2])
end
