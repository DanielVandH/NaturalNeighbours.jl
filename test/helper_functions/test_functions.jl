...
function plot_2d_vals(fig, method, xp, yp, vals, errs, ε, i, ij, a, b, c, d, xg, yg, levels, x1, x2, title, show_scatter, x, y, z1, z2, z3)
    ...
    xlims!(ax, a, b)
    ylims!(ax, c, d)
    m = contourf!(ax, xg, yg, reshape(vals, length(xg), length(yg)), levels=sort(levels), extendlow=:auto, extendhigh=:auto)
    if show_scatter
...
function complete_test_function_analysis(id)
    ...
    for ((derivative_method, interpolant_method, (xp, yp)), (vals, errs, ε)) in first_order_gradient_results
        ...
        c = min(c, minimum(yp))
        d = max(d, maximum(yp))
        nΔ = filter(x -> !any(x -> isnan(x) || isinf(x), x), vals)
        append!(all_∇, norm.(nΔ))
        append!(all_errs, filter(x -> !(isnan(x) || isinf(x)), errs))
    end
    zmin = 0.0
